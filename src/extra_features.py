...
def calculate_richardson_feature(data, newFeatureName):
    ...
    min_val = float(data[0][newFeatureName])
    max_val = float(data[0][newFeatureName])
    for elem in data:
        min_val = min((min_val, float(elem[newFeatureName])))
        max_val = max((max_val, float(elem[newFeatureName])))

    for elem in data:
        elem[newFeatureName + "_normalized"] = (float(elem[newFeatureName]) - min_val) / (max_val - min_val)

def calculate_richardson_ln_feature(data, newFeatureName):
    # Normal Richardson: I = 0.013 * EXP(0.0036 * V - connection_angle^2 / (2 * sigma^2))
    # Our feature version: LN(I) = LN(0.013 * EXP(-connection_angle^2 / (2 * sigma^2))) = LN(0.013) + (-connection_angle)^2 / (2 * sigma^2) = connection_angle^2
    # Constants get absorbed into weights
    # Expect connection_angle to be DEGREES
    # sigma = 43
    for elem in data:
        connection_angle = elem["connection_angle_degrees"]
        richardson_ln_feature = math.pow(connection_angle, 2)
        elem[newFeatureName] = richardson_ln_feature
...
def main():
    ...
    fieldnames.append("phi_2_degrees")
    ...
    fieldnames.append(richardson_feature_name + "_normalized")

    richardson_ln_feature_name = "richardson_formula_degrees_ln"
    calculate_richardson_ln_feature(data, richardson_ln_feature_name)
    fieldnames.append(richardson_ln_feature_name)

    fieldnames.append(richardson_ln_feature_name + "_normalized")
