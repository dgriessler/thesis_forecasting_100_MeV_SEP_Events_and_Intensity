...
class EvalClassifier(object):
    ...
    def plotEvalGraphScaled(self, y_true, predictions, y_target, filename, minX, maxX, minY, maxY, plotTitle="", xLabel="Classification", yLabel="Score", threshold=0.5):
        ...
        for i in range(0, len(y_target)):
            ...
            if y_true[i] == 1:
                actual_y_true.append("yes")
            else:
                actual_y_true.append("no")

        # Plot y-axis as predicted value and x-axis as original value

        fig, ax = plt.subplots()

        ax.scatter(actual_y_true, predictions, c=scatterColors)
    ...
    def regNN(self, pred_threshold=0.5):
        ...
        adam_model_001_1_retrained, adam_history_001_1_retrained = self.cla.regNN(retrained_adamLearningRate, all_trainingData_x, all_trainingData_y, retrained_logFilename, seed=1234, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename)
        ...
        minX, maxX, minY, maxY = self.plotEvalGraph(testData_y, predictedData_y, testData_target, "{}/threshold_{}_Predictions_Reg_NN_F1_{}.png".format(evalFolder, pred_threshold, regNN_F1), threshold=pred_threshold)
    ...
    def regNN_iterate(self, pred_threshold=0.5):
        ...
        for j in range(0, numIterations):
            ...
            adam_model_001_1_retrained, _ = self.cla.regNN(retrained_adamLearningRate, None, None, None, seed=1234+j, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename)
            ...
            minX, maxX, minY, maxY = self.plotEvalGraph(testData_y, predictedData_y, testData_target, "{}.png".format(evalFilename), threshold=pred_threshold)
            indexes = get_indexes("{}/firstStageTest.csv".format(resFolder))
    ...
    def regNN_oversampled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha, pred_threshold=0.5):
        ...
        outFolder = "../out/classifier/regNN_oversampled_0_{}".format(folder_trailer)
        ...
        for j in range(0, numIterations):
            weights_filename = "{}/adam_model_001_01_oversampled_0_{}_retrained_it_{}.ckpt".format(outFolder, folder_trailer, j)
            print("WEIGHTS_FILENAME: {}".format(weights_filename))
            NNModel, _ = self.cla.regNN(adamLearningRate, None, None, None, seed=1234+j, adamEpsilon=adam_epsilon, alpha=alpha, weights_filename=weights_filename)
    ...
    def rRT(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha, pred_threshold=0.5):
        ...
        for j in range(0, numIterations):
            ...
            adam_model_001_1_retrained, _ = self.cla.regNN(retrained_adamLearningRate, None, None, None, seed=1234, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename)

            NNModel, _ = self.cla.rRT(adamLearningRate, adam_model_001_1_retrained, None, None, None, seed=1234+j, adamEpsilon=adam_epsilon, alpha=alpha, weights_filename=weights_filename)
            predictedData_y = NNModel.predict(testData_x)
...
if __name__ == "__main__":
    ...
    for over_amount in range(1, 10):
        ...
        for over_amount_round in range(0, 9):
            ...
            alpha = 0.3
            evaluator.regNN_oversampled(oversample_amount, "secondStageOversampleTest_percentSEP_0.{}.csv".format(over_amount), adamLearningRate, alpha=alpha, pred_threshold=pred_threshold)

            adamLearningRate = 0.001
            alpha = 0.3
            evaluator.rRT(oversample_amount, "secondStageOversampleTest_percentSEP_0.{}.csv".format(over_amount), adamLearningRate, alpha=alpha, pred_threshold=pred_threshold)

            adamLearningRate = 0.0001
            evaluator.autoencoder(oversample_amount, "secondStageOversampleTest_percentSEP_0.{}.csv".format(over_amount), adamLearningRate, pred_threshold=pred_threshold)
