...
class Regression(object):
    def __init__(self):
        self.lossWeightAlpha = 1.612578057
    ...
    def denseLossNNModelValidation(self, adamLearningRate, trainingData_x, trainingData_y, validationData, logFilename, denseLossAlpha, adamEpsilon, alpha, numEpochs=3000, weights_filename=None):
        print("Adam Learning Rate: {:.5f}. Adam epsilon: {:.5f}. Num epochs: {:}. Alpha: {:.5f}.".format(adamLearningRate, adamEpsilon, numEpochs, alpha))
    ...
    def learnedRichardsonAlongsideRegularNNValidation(self, adamLearningRate, trainingData_x, trainingData_y, validationData, logFilename, adamEpsilon, alpha, numEpochs=3000, weights_filename=None, doTrain = True):
        ...
        inputLayerReg = keras.Input(shape=(20,))

        # Hidden layer regular NN
        encoderLayerReg = keras.layers.Dense(18, kernel_initializer=initializer, activation=LeakyReLU(alpha=alpha))(inputLayerReg)
