...
class EvalRegression(object):
    ...
    def regNN(self):
        ...
        regNN_corr, regNN_meanAbsoluteErrorSEP, regNN_FP, regNN_FN, regNN_TP, regNN_TN, regNN_F1, regNN_HSS, regNN_TSS = self.evaluate(testData_y, predictedData_y, testData_target)
    ...
    def regNN_iterate(self):
        ...
        for j in range(0, numIterations):
            ...
            predictedData_y = adam_model_001_1_retrained.predict(testData_x)
            regNN_corr, regNN_mae, regNN_FP, regNN_FN, regNN_TP, regNN_TN, regNN_F1, regNN_HSS, regNN_TSS = self.evaluate(testData_y, predictedData_y, testData_target)
    ...
    def regNN_oversampled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha):
        ...
        HSSMean = HSSMean / numIterations
        TSSMean = TSSMean / numIterations
        print("reg,{},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f}".format(oversampleAmount, corrMean, maeMean, FPMean, FNMean, TPMean, TNMean, F1Mean, HSSMean, TSSMean))
    ...
    def rRT(self, oversampleAmount, testDataFilename, adamLearningRate, adamEpsilon, alpha):
        ...
        for j in range(0, numIterations):
            ...
            predictedData_y = NNModel.predict(testData_x)
            rRT_corr, rRT_mae, rRT_FP, rRT_FN, rRT_TP, rRT_TN, rRT_F1, rRT_HSS, rRT_TSS = self.evaluate(testData_y, predictedData_y, testData_target)

            corrMean = corrMean + rRT_corr
            maeMean = maeMean + rRT_mae
        ...
        HSSMean = HSSMean / numIterations
        TSSMean = TSSMean / numIterations
        print("rRT,{},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f}".format(oversampleAmount, corrMean, maeMean, FPMean, FNMean, TPMean, TNMean, F1Mean, HSSMean, TSSMean))
    ...
    def autoencoder(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha):
        ...
        for j in range(0, numIterations):
            ...
            aut_corr, aut_mae, aut_FP, aut_FN, aut_TP, aut_TN, aut_F1, aut_HSS, aut_TSS = self.evaluate(testData_y, predictedData_y, testData_target)

            corrMean = corrMean + aut_corr
            maeMean = maeMean + aut_mae
        ...
        HSSMean = HSSMean / numIterations
        TSSMean = TSSMean / numIterations
        print("aut,{},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f},{:.2f}".format(oversampleAmount, corrMean, maeMean, FPMean, FNMean, TPMean, TNMean, F1Mean, HSSMean, TSSMean))
