...
def showHistoryValidation(history, show=False):
    ...
    if show:
        plt.title("Loss (Mean Squared Error) and Validation Loss VS Epoch")
        plt.xlabel("Epoch")
        plt.scatter(xAxisValues, yAxisValuesLoss, label="Loss (Mean Squared Error)")
...
def showHistory(history, historyKey="loss", plotTitle="Loss (Mean Squared Error) VS Epoch", plotYLabel="Loss (Mean Squared Error)", show=False):
    if history is None:
        print("History is None...")
        return
...
def regNNModelValidationWithAdamAndLeakyReLU(adamLearningRate, trainingData_x, trainingData_y, validationData, logFilename, adamEpsilon, alpha, numEpochs=3000, weights_filename=None):
...
def regNNModelValidationWithAdamAndLeakyReLUCont(model, trainingData_x, trainingData_y, validationData, logFilename, numEpochs=3000, weights_filename=None):
    if weights_filename is not None:
        model.load_weights(weights_filename)
        history = None
    else:
...
def regNNModelWithAdamAndLeakyReLU(adamLearningRate, all_trainingData_x, all_trainingData_y, logFilename, seed, adamEpsilon, alpha, numEpochs=150, weights_filename=None):
    print("Adam Learning Rate: {:.5f}. Adam epsilon: {:.5f}. Num epochs: {:}. Alpha: {:.5f}.".format(adamLearningRate, adamEpsilon, numEpochs, alpha))
...
def plotEvaluation(y_true, predictions, y_target, show=False):
    minX = min(y_true) - 0.1
    maxX = max(y_true) + 0.1
    ...
    if show:
        ...
        ax.text(4, 2.5, "Mean Absolute Error on SEP: {:.2f}".format(meanAbsoluteErrorSEP))
    ...
    return (corr, meanAbsoluteErrorSEP, FP, FN, TP, TN, F1, HSS, TSS)

def plotEvaluationLN(y_true, predictions, y_target, show=False):
    minX = min(y_true) - 0.1
    maxX = max(y_true) + 0.1
    minY = min(predictions) - 0.1
    maxY = max([max(predictions) + 0.1, 1.1])
...
def rRTValidationAndLeakyReLU(adamLearningRate, model, trainingData_x, trainingData_y, validationData, logFilename, adamEpsilon, alpha, numEpochs=3000, weights_filename=None, do_train=True):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
...
def rRTAndLeakyReLURecall(adamLearningRate, model, weights_filename, adamEpsilon, alpha):
...
def autoencoder_finding_alpha(optimizer, autoencoder_trainingData_x, autoencoder_trainingData_y, autoencoder_validationData, logFilename, logFilenameClassifier, numEpochs, weights_filename, weights_filename_classifier, alpha=0.3):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
    ...
    modelSecond.compile(optimizer=optimizer, loss=keras.losses.MeanSquaredError())
...
def autoencoder(optimizer, lossWeightAlpha, logFilename, trainingData_x, trainingData_y, validationData, numEpochs, weights_filename=None, alpha=0.3):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
    ...
    model.compile(optimizer=optimizer, loss=[keras.losses.MeanSquaredError(),keras.losses.MeanSquaredError()],loss_weights=[lossWeightAlpha,1])
...
def autoencoderAll(optimizer, lossWeightAlpha, logFilename, trainingData_x, trainingData_y, numEpochs, weights_filename=None, alpha=0.3):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
...
def autoencoder_second_stage(autoencoder_validation_weights_path, optimizer, numEpochs, autoencoder_trainingData_x, autoencoder_trainingData_y, validationData, logFilename, lossWeightAlpha, weights_filename=None, alpha=0.3):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
    ...
    autoencoder_first_stage.compile(optimizer=optimizer, loss=[keras.losses.MeanSquaredError(),keras.losses.MeanSquaredError()],loss_weights=[lossWeightAlpha,1])
    ...
    autoencoder_second_stage_model.compile(optimizer=optimizer, loss=keras.losses.MeanSquaredError())
...
def autoencoder_second_stage_all(autoencoder_validation_weights_path, optimizer, numEpochs, autoencoder_trainingData_x, autoencoder_trainingData_y, logFilename, lossWeightAlpha, weights_filename=None, alpha=0.3, train=True):
    initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
