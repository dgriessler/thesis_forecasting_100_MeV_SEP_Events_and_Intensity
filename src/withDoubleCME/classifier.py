from base_func_classifier import *

class Classifier(object):
    def __init__(self):
        self.lossWeightAlpha = 0.03306296
    ...
    def regNN(self, adamLearningRate, all_trainingData_x, all_trainingData_y, logFilename, seed=getNextSeed(), adamEpsilon, alpha, numEpochs=150, callbacks=[], weights_filename=None):
        print("Adam Learning Rate: {:.5f}. Adam epsilon: {:.5f}. Num epochs: {:}. Alpha: {:.5f}.".format(adamLearningRate, adamEpsilon, numEpochs, alpha))
    ...
    def rRTValidation(self, adamLearningRate, model, trainingData_x, trainingData_y, validationData, logFilename, adamEpsilon, alpha, numEpochs=3000, callbacks=[], weights_filename=None, do_train=True):
        initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
    ...
    def autoencoder_finding_alpha(self, optimizer, optimizerClassifier, autoencoder_trainingData_x, autoencoder_trainingData_y, autoencoder_validationData, logFilename, logFilenameClassifier, numEpochs, weights_filename, weights_filename_classifier, alpha):
        initializer = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=getNextSeed())
    ...
    def autoencoder_second_stage_all(self, autoencoder_validation_weights_path, optimizer, optimizer_ss, numEpochs, autoencoder_trainingData_x, autoencoder_trainingData_y, logFilename, alpha, weights_filename=None, seed=None, train=True):
        if seed is None:
        ...
        initializer_ss = keras.initializers.RandomUniform(minval=-0.05, maxval=0.05, seed=next_seed)
