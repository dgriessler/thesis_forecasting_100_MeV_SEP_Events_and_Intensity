...
class EvalRichardson(object):
    ...
    def richardsonOriginal(self):
        ...
        for group in range(0, 3):
            ...
            if minXRichardson is None or newMaxes[0] < minXRichardson:
                minXRichardson = newMaxes[0]
            if maxXRichardson is None or newMaxes[1] > maxXRichardson:
                maxXRichardson = newMaxes[1]
            if minYRichardson is None or newMaxes[2] < minYRichardson:
                minYRichardson = newMaxes[2]
            if maxYRichardson is None or newMaxes[3] > maxYRichardson:
    ...
    def richardsonOriginalEvaluate(self):
        evalReg = EvalRegression()
        group = 0
        idxes = get_indexes("../res/gen/fold/firstStageTest_3Fold_{}.csv".format(group))

        print(idxes)

        expected_richardson_x, one_hundred_mev_y = self.getExpectedRichardson(idxes)
        testData_target = self.reg.getTargets("../res/gen/fold/firstStageTest_3Fold_{}.csv".format(group))
        orig_corr_sep, orig_corr_sep_elevated, orig_meanAbsoluteErrorSEP, orig_meanAbsoluteErrorAll, orig_FP, orig_FN, orig_TP, orig_TN, orig_F1, orig_HSS, orig_TSS = evalReg.evaluate(one_hundred_mev_y, expected_richardson_x, testData_target)

        overallMinX, overallMaxX, overallMinY, overallMaxY = self.plotEvalGraph(one_hundred_mev_y, expected_richardson_x, testData_target, "../eval/expected_richardson/EntireDataset_ExpectedRichardson_{}.png".format(group), plotTitle="Richardson Formula MeV Peak Intensity LN vs 100 MeV Peak Intensity LN", xLabel="100 MeV Peak Intensity LN", yLabel="Richardson Formula MeV Peak Intensity LN")

        adamLearningRate = 0.0001
        adamEpsilon = 1.0
        retrained_weights_filename = "../out/retrained_learn_richardson_on_syn_3fold/group_{}_lR_{:.5f}.ckpt".format(group, adamLearningRate)
        adam_model_001_1_retrained, adam_history_001_1_retrained = self.reg.trainRichardson(adamLearningRate, None, None, None, adamEpsilon=adamEpsilon, weights_filename=retrained_weights_filename)

        testData_x, testData_y  = self.reg.getFirstStageInputRichardson("../res/gen/fold/firstStageTest_3Fold_{}.csv".format(group), useSynData=False)
        predictedData_y = adam_model_001_1_retrained.predict(testData_x)
        learned_corr_sep, learned_corr_sep_elevated, learned_meanAbsoluteErrorSEP, learned_meanAbsoluteErrorAll, learned_FP, learned_FN, learned_TP, learned_TN, learned_F1, learned_HSS, learned_TSS = evalReg.evaluate(testData_y, predictedData_y, testData_target)

        learned_min_x, learned_max_x, learned_min_y, learned_max_y = self.plotEvalGraph(testData_y, predictedData_y, testData_target, "../eval/retrained_learn_richardson_on_syn_3fold/EntireDataset_Predictions_Reg_NN_{}.png".format(group), plotTitle="Predicted 100 MeV Peak Intensity LN vs 100 MeV Peak Intensity LN", xLabel="100 MeV Peak Intensity LN", yLabel="Predicted 100 MeV Peak Intensity LN")

        overallMinX = min((overallMinX, learned_min_x)) - 0.1
        overallMaxX = max((overallMaxX, learned_max_x)) + 0.1
        overallMinY = min((overallMinY, learned_min_y)) - 0.1
        overallMaxY = max((overallMaxY, learned_max_y)) + 0.1

        self.plotEvalGraphScaled(one_hundred_mev_y, expected_richardson_x, testData_target, "../eval/expected_richardson/EntireDataset_ExpectedRichardson_{}_Scaled.png".format(group), overallMinX, overallMaxX, overallMinY, overallMaxY, plotTitle="Richardson Formula MeV Peak Intensity LN vs 100 MeV Peak Intensity LN", xLabel="100 MeV Peak Intensity LN", yLabel="Richardson Formula MeV Peak Intensity LN")
        self.plotEvalGraphScaled(testData_y, predictedData_y, testData_target, "../eval/retrained_learn_richardson_on_syn_3fold/EntireDataset_Predictions_Reg_NN_{}_Scaled.png".format(group), overallMinX, overallMaxX, overallMinY, overallMaxY, plotTitle="Predicted 100 MeV Peak Intensity LN vs 100 MeV Peak Intensity LN", xLabel="100 MeV Peak Intensity LN", yLabel="Predicted 100 MeV Peak Intensity LN")

        print("Approach & FP & FN & TP & TN & F1 & HSS & TSS\\\\")
        print("Original & {:.1f} & {:.1f} & {:.1f} & {:.1f} & {:.3f} & {:.3f} & {:.3f}\\\\".format(orig_FP, orig_FN, orig_TP, orig_TN, orig_F1, orig_HSS, orig_TSS))
        print("Learned & {:.1f} & {:.1f} & {:.1f} & {:.1f} & {:.3f} & {:.3f} & {:.3f}\\\\".format(learned_FP, learned_FN, learned_TP, learned_TN, learned_F1, learned_HSS, learned_TSS))
    ...
    def learnRichardsonSyn3Fold(self):
        ...
        for i in range(0, numFolds):
            ...
            dataset_weights[i]['w_v'] = w_v
            w_0 = layer0Weights[1][0]
            w_exp = math.exp(w_0)
            print("\n\n\nW_0: {}. {}\n\n\n".format(w_0, w_exp))
            dataset_weights[i]['w_exp'] = w_exp

            fieldnames = []
            for key in dataset_weights[i].keys():
                fieldnames.append(key)

            csvFilename = "../res/trained/richardson_{}.csv".format(i)
            with open(csvFilename, 'w', newline='') as csvfile:
                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerow(dataset_weights[i])
...
def main():
    ...
    #print("\n\ndataset_weights\n")
    #for index in range(0, len(dataset_weights)):
    #    print("weights: {}".format(index))
    #    for key in dataset_weights[index].keys():
    #        print("KEY: {}. VAL: {}".format(key, dataset_weights[index][key]))
            
    #for index in range(0, len(dataset_weights)):
    #    csvFilename = "../res/trained/richardson_{}.csv".format(index)
    #    with open(csvFilename, 'w', newline='') as csvfile:
    #            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    #            writer.writeheader()
    #            writer.writerow(dataset_weights[index])
    ...
    return (overallMinX, overallMaxX, overallMinY, overallMaxY)

if __name__ == "__main__":
    #overallMinX, overallMaxX, overallMinY, overallMaxY = main()
    evaluator = EvalRichardson()
    evaluator.richardsonOriginalEvaluate()
