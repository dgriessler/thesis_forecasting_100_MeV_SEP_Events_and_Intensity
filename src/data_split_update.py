...
def update_first_stage_training(data):
    ...
    index_map_data = mapByIndex(normalized_data)
...
def update_stratified_richardson(data):
    ...
    normalized_index_map_data = mapByIndex(normalized_data)
    ...
    for k in range(0, 3):
        for j in range(0, 4):
            ...
            for filename in filenames:
                d = readCSVFile(filename)
                for elem in d:
                    new_elem = normalized_index_map_data[int(elem["index"])]
                    if new_elem is not None:
                        rem_keys = []
                        for key in elem.keys():
                            if key in new_elem.keys():
