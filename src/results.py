...
class Results(object):
    def __init__(self):
        ...
        for key in self.stats:
            for i in range(0, 10):
                self.stats[key][i*10] = dict()
                self.stats[key][i*10]["min_x"] = None
                self.stats[key][i*10]["max_x"] = None
                self.stats[key][i*10]["min_y"] = None
                self.stats[key][i*10]["max_y"] = None
    ...
    def updateMinX(self, t, oversampleAmount, min_x):
        if self.stats[t][oversampleAmount]["min_x"] is None:
            self.stats[t][oversampleAmount]["min_x"] = min_x
        else:
            self.stats[t][oversampleAmount]["min_x"] = min(self.stats[t][oversampleAmount]["min_x"], min_x)

    def updateMaxX(self, t, oversampleAmount, max_x):
        if self.stats[t][oversampleAmount]["max_x"] is None:
            self.stats[t][oversampleAmount]["max_x"] = max_x
        else:
            self.stats[t][oversampleAmount]["max_x"] = max(self.stats[t][oversampleAmount]["max_x"], max_x)

    def updateMinY(self, t, oversampleAmount, min_y):
        if self.stats[t][oversampleAmount]["min_y"] is None:
            self.stats[t][oversampleAmount]["min_y"] = min_y
        else:
            self.stats[t][oversampleAmount]["min_y"] = min(self.stats[t][oversampleAmount]["min_y"], min_y)

    def updateMaxY(self, t, oversampleAmount, max_y):
        if self.stats[t][oversampleAmount]["max_y"] is None:
            self.stats[t][oversampleAmount]["max_y"] = max_y
        else:
            self.stats[t][oversampleAmount]["max_y"] = max(self.stats[t][oversampleAmount]["max_y"], max_y)

    def getMinX(self, t, oversampleAmount):
        return self.stats[t][oversampleAmount]["min_x"]

    def getMaxX(self, t, oversampleAmount):
        return self.stats[t][oversampleAmount]["max_x"]

    def getMinY(self, t, oversampleAmount):
        return self.stats[t][oversampleAmount]["min_y"]

    def getMaxY(self, t, oversampleAmount):
        return self.stats[t][oversampleAmount]["max_y"]
    ...
    def metrics_to_string(self, t, oversampleAmount):
        ...
        for key in ["corrSEPMean", "corrSEPElevatedMean", "maeSEPMean", "maeAllMean", "FPMean", "FNMean", "TPMean", "TNMean", "F1Mean", "HSSMean", "TSSMean"]:
            if key in m:
                s = s + ",{:2f}".format(m[key])
        s = s + "," + str(self.getMinX(t, oversampleAmount))
        s = s + "," + str(self.getMaxX(t, oversampleAmount))
        s = s + "," + str(self.getMinY(t, oversampleAmount))
        s = s + "," + str(self.getMaxY(t, oversampleAmount))
        return s
