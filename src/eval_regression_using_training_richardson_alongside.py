...
class EvalRegressionUsingTrainingRichardsonAlongside(EvalRegression):
    ...
    def regNN(self):
        resFolder = "../res/gen"
        outFolder = "../out/using_training_richardson_alongside/regNN_retrained"
        evalFolder = "../eval/using_training_richardson_alongside/regNN_retrained"

        all_trainingData_x, all_trainingData_y = self.reg.getFirstStageInput("{}/firstStageAllTraining.csv".format(resFolder))

        richardson_trainingData_x, richardson_trainingData_y = self.reg.getFirstStageInputRichardson("{}/firstStageAllTraining.csv".format(resFolder), False)

        combined_training_data_x = (richardson_trainingData_x, all_trainingData_x)
        combined_training_data_y = all_trainingData_y

        adam_model_001_1_retrained_checkpoint_path = "{}/adam_model_001_1.ckpt".format(outFolder)

        retrained_adamLearningRate = 0.001
        retrained_adamEpsilon = 1.0
        retrained_alpha = 0.3

        retrained_logFilename = "{}/adam_model_001_1.csv".format(outFolder)

        retrained_weights_filename = adam_model_001_1_retrained_checkpoint_path

        adam_model_001_1_retrained, adam_history_001_1_retrained = self.reg.learnedRichardsonAlongsideRegularNN(retrained_adamLearningRate, combined_training_data_x, combined_training_data_y, retrained_logFilename, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename, doTrain = False)

        # Evaluate
        testData_x, testData_y  = self.reg.getFirstStageInput("{}/firstStageTest.csv".format(resFolder))
        richardson_testData_x, richardson_testData_y = self.reg.getFirstStageInputRichardson("{}/firstStageTest.csv".format(resFolder), False)

        combined_testData_x = (richardson_testData_x, testData_x)
        combined_testData_y = testData_y
        testData_target = self.reg.getTargets("{}/firstStageTest.csv".format(resFolder))

        predictedData_y = adam_model_001_1_retrained.predict(combined_testData_x)
        regNN_corr_sep, regNN_corr_sep_elevated, regNN_meanAbsoluteErrorSEP, regNN_meanAbsoluteErrorAll, regNN_FP, regNN_FN, regNN_TP, regNN_TN, regNN_F1, regNN_HSS, regNN_TSS = self.evaluate(combined_testData_y, predictedData_y, testData_target)
    ...
    def regNN_iterate(self):
        resFolder = "../res/gen"
        outFolder = "../out/using_training_richardson_alongside/regNN_retrained"
        evalFolder = "../eval/using_training_richardson_alongside/regNN_retrained/iterate"
        ...
        for j in range(0, numIterations):
            ...
            adam_model_001_1_retrained, _ = self.reg.learnedRichardsonAlongsideRegularNN(retrained_adamLearningRate, None, None, None, seed=1234+j, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename, doTrain = False)
            ...
            minX, maxX, minY, maxY = self.plotEvalGraph(combined_testData_y, predictedData_y, testData_target, "{}.png".format(evalFilename), plotTitle="Predicted Peak Intensity LN vs Original Peak Intensity LN", xLabel="Peak Intensity LN", yLabel="Predicted Peak Intensity LN")
            indexes = get_indexes("{}/firstStageTest.csv".format(resFolder))
            self.outputPredictions(get_real_data(self.orig_data, indexes), combined_testData_y, predictedData_y, testData_target, "{}.csv".format(evalFilename))
    ...
    def regNN_oversampled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha):
        ...
        outFolder = "../out/using_training_richardson_alongside/retrained_regNN_oversampled_0_{}".format(folder_trailer)
        evalFolder = "../eval/using_training_richardson_alongside/retrained_regNN_oversampled_0_{}".format(folder_trailer)
        ...
        richardson_testData_x, richardson_testData_y = self.reg.getFirstStageInputRichardson("{}/{}".format(resFolder, testDataFilename), False)
        combined_testData_x = (richardson_testData_x, testData_x)
        combined_testData_y = testData_y
        ...
        for j in range(0, numIterations):
            ...
            NNModel, _ = self.reg.learnedRichardsonAlongsideRegularNN(adamLearningRate, None, None, None, seed=1234+j, adamEpsilon=adam_epsilon, alpha=alpha, weights_filename=weights_filename, doTrain = False)
            predictedData_y = NNModel.predict(combined_testData_x)
            regNN_corr_sep, regNN_corr_sep_elevated, regNN_meanAbsoluteErrorSEP, regNN_meanAbsoluteErrorAll, reg_FP, reg_FN, reg_TP, reg_TN, reg_F1, reg_HSS, reg_TSS = self.evaluate(combined_testData_y, predictedData_y, testData_target)
    ...
    def regNN_oversampled_scaled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha, min_x, max_x, min_y, max_y):
        ...
        for j in range(0, numIterations):
            ...
            self.plotEvalGraphScaled(combined_testData_y, predictedData_y, testData_target, "{}.png".format(evalFilename), min_x, max_x, min_y, max_y, plotTitle="Predicted Peak Intensity LN vs Original Peak Intensity LN", xLabel="Peak Intensity LN", yLabel="Predicted Peak Intensity LN")
    ...
    def rRT(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha):
        resFolder = "../res/gen"
        folder_trailer = int(oversampleAmount / 10)
        outFolder = "../out/using_training_richardson_alongside/retrained_rRT_0_{}".format(folder_trailer)
        evalFolder = "../eval/using_training_richardson_alongside/retrained_rRT_0_{}".format(folder_trailer)
        ...
        for j in range(0, numIterations):
            ...
            retrained_weights_filename = "../out/using_training_richardson_alongside/regNN/adam_model_001_1.ckpt"
            adam_model_001_1, _ = self.reg.learnedRichardsonAlongsideRegularNN(retrained_adamLearningRate, None, None, None, adamEpsilon=retrained_adamEpsilon, alpha=retrained_alpha, weights_filename=retrained_weights_filename, doTrain=False)

            feature_extractor = get_feature_extractor(adam_model_001_1, lastHiddenLayerIndex=len(adam_model_001_1.layers) - 2)

            NNModel, _ = self.reg.learnedRichardsonAlongsideRRT(adamLearningRate, feature_extractor, None, None, None, adamEpsilon=adam_epsilon, alpha=alpha, seed=1234+j, weights_filename=weights_filename, do_train=False)

            predictedData_y = NNModel.predict(combined_testData_x)
            rRT_corr_sep, rRT_corr_sep_elevated, rRT_meanAbsoluteErrorSEP, rRT_meanAbsoluteErrorAll, rRT_FP, rRT_FN, rRT_TP, rRT_TN, rRT_F1, rRT_HSS, rRT_TSS = self.evaluate(combined_testData_y, predictedData_y, testData_target)
    ...
    def rRT_scaled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha, min_x, max_x, min_y, max_y):
        resFolder = "../res/gen"
        folder_trailer = int(oversampleAmount / 10)
        outFolder = "../out/using_training_richardson_alongside/retrained_rRT_0_{}".format(folder_trailer)
        evalFolder = "../eval/using_training_richardson_alongside/retrained_rRT_0_{}".format(folder_trailer)
    ...
    def autoencoder(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha):
        resFolder = "../res/gen"
        folder_trailer = int(oversampleAmount / 10)
        outFolder = "../out/using_training_richardson_alongside/retrained_autoencoder_ss_0_{}".format(folder_trailer)
        evalFolder = "../eval/using_training_richardson_alongside/retrained_autoencoder_ss_0_{}".format(folder_trailer)
        ...
        for j in range(0, numIterations):
            ...
            autoencoder_second_stage_model = self.reg.learnedRichardsonAlongside_autoencoder_second_stage_all(autoencoder_validation_weights_path, adamOptimizer, adamOptimizerSS, None, None, None, None, alpha=alpha, weights_filename=weights_filename, train=False, seed=1234+j)
            predictedData_y = autoencoder_second_stage_model.predict(combined_testData_x)
            aut_corr_sep, aut_corr_sep_elevated, aut_meanAbsoluteErrorSEP, aut_meanAbsoluteErrorAll, aut_FP, aut_FN, aut_TP, aut_TN, aut_F1, aut_HSS, aut_TSS = self.evaluate(combined_testData_y, predictedData_y, testData_target)
    ...
    def autoencoder_scaled(self, oversampleAmount, testDataFilename, adamLearningRate, adam_epsilon, alpha, x_min, x_max, y_min, y_max):
        resFolder = "../res/gen"
        folder_trailer = int(oversampleAmount / 10)
        outFolder = "../out/using_training_richardson_alongside/retrained_autoencoder_ss_0_{}".format(folder_trailer)
        evalFolder = "../eval/using_training_richardson_alongside/retrained_autoencoder_ss_0_{}".format(folder_trailer)
        ...
        for j in range(0, numIterations):
            ...
            self.plotEvalGraphScaled(combined_testData_y, predictedData_y, testData_target, "{}.png".format(evalFilename), x_min, x_max, y_min, y_max, plotTitle="Predicted Peak Intensity LN vs Original Peak Intensity LN", xLabel="Peak Intensity LN", yLabel="Predicted Peak Intensity LN")

        seenF1Scores.clear()

if __name__ == "__main__":
    evaluator = EvalRegressionUsingTrainingRichardsonAlongside()
    evaluator.regNN()
    ...
    adamEpsilon = 1.0
    alpha = 0.3
    evaluator.regNN_oversampled_scaled(60, "secondStageOversampleTest_percentSEP_0.{}.csv".format(6), adamLearningRate, adamEpsilon, alpha, min_x, max_x, min_y, max_y)

    adamLearningRate = 0.001
    adamEpsilon = 1.0
    alpha = 0.3
    evaluator.rRT_scaled(30, "secondStageOversampleTest_percentSEP_0.{}.csv".format(3), adamLearningRate, adamEpsilon, alpha, min_x, max_x, min_y, max_y)

    adamLearningRate = 0.001
    adamEpsilon = 1.0
    alpha = 0.3
    evaluator.autoencoder_scaled(20, "secondStageOversampleTest_percentSEP_0.{}.csv".format(2), adamLearningRate, adamEpsilon, alpha, min_x, max_x, min_y, max_y)
